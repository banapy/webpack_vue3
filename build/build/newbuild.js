...
// process.chdir(path.resolve("./node_modules/@vue/cli-service/lib/commands/build"));
...
async function build(args, api, options) {
	...
	let webpackConfig;
	if (args.target === "lib") {
		webpackConfig = require("@vue/cli-service/lib/commands/build/resolveLibConfig")(api, args, options);
	} else if (args.target === "wc" || args.target === "wc-async") {
		webpackConfig = require("@vue/cli-service/lib/commands/build/resolveWcConfig")(api, args, options);
	} else {
		webpackConfig = require("@vue/cli-service/lib/commands/build/resolveAppConfig")(api, args, options);
	}
	...
	if (args.report || args["report-json"]) {
		...
		modifyConfig(webpackConfig, (config) => {
			const bundleName =
				args.target !== "app" ? config.output.filename.replace(/\.js$/, "-") : isLegacyBuild ? "legacy-" : "";
			config.plugins.push(
	...
	return new Promise((resolve, reject) => {
		webpack([workerConfig,webpackConfig,], (err, stats) => {
			stopSpinner(false);
			if (err) {
				return reject(err);
			}
